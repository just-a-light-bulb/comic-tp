...
| **Role** | **Font & Rationale** |
|---|---|
| **Display / H1** | Shippori Mincho B1 (Google Fonts) — high-contrast Japanese-influenced serif. Used for page titles, project names, chapter numbers. |
| **Headings / H2–H3** | Cormorant Garamond Bold — refined high-contrast Western serif. Used for section headings, panel headers, card titles. |
| **Body text / Labels** | Noto Serif — universal serif supporting all scripts including Thai. Used for all body copy, table content, form labels. |
| **UI Controls / Tags** | DM Mono — geometric monospaced font for badges, confidence scores, keyboard shortcut labels, and code blocks. |
| **Thai text rendering** | Noto Serif Thai — must be loaded explicitly alongside Noto Serif so Thai characters render with correct weight and baseline. |

**⚠️ Font Loading Strategy**

Load all fonts via Google Fonts CDN with `display=swap`. Add `preconnect` tags for `fonts.googleapis.com` and `fonts.gstatic.com` in `app.html` before the font link element.

---

### B.4 Spacing & Layout Scale

Base unit: 4px (`var(--unit)`). All spacing values are multiples: 4, 8, 12, 16, 24, 32, 48, 64, 96. Exception: the canvas panel uses pixel-exact positioning driven by image dimensions.
...
body {
  ...
  background-image: radial-gradient(circle, var(--tone-gray) 1px, transparent 1px);
...
|---|---|
| **/ (Landing)** | Full-width manga-panel grid layout. Four panels show screenshots with caption boxes. Hero headline: Shippori Mincho 64px. CTA button: panel-red. Halftone background with screentone gradient fading to white at bottom. |
| **/projects (List)** | Masonry grid (3 cols desktop, 2 tablet, 1 mobile). Each card: cover thumbnail, Shippori Mincho title, JP → TH language pair, chapter count + last-updated in DM Mono. Hover lifts with box-shadow press effect. |
| **/projects/[pid] (Detail)** | Chapter cards 2-column grid. Each card: 3-page thumbnail strip, large Cormorant chapter numeral, ink-stroke progress bar. |
| **/projects/[pid]/chapters/[cid] (Dashboard)** | Scrollable 4-column page thumbnail grid. Each thumbnail: speech-bubble status badge bottom-right (pending / processing / translated / done). Sticky toolbar: Translate All + Export buttons. |
| **/editor/[pageId] (Main Editor)** | Two-panel layout with draggable resize handle. Left: Canvas. Right: Translation Spreadsheet. Both panel-frame containers. Top bar: breadcrumb in Shippori Mincho + Render/Export buttons. Speed-line divider below top bar. |
| **/settings** | Single-column form. API key field with show/hide toggle. Storage meter as ink-stroke bar (not rounded pill). |
